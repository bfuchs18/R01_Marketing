...
#
# The purpose of this script is to create and run 1sample t-tests other that Food vs Toy that validate basic patterns to food cues
...
# set test_dir 
set test_dir = $lev2_dir/1sampleT_HvL
...
foreach map ( $maps )
	...
	# generate ttest script --- no covariates
	gen_group_command.py -command 3dttest++                                             \
...
### REML ####
set reml_maps = ( image_HEDvLED )

foreach map ( $reml_maps )
	set output_name = $map
