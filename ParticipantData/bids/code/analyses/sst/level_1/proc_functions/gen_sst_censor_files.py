...
def gen_sst_censor_files(sub, fmriprep_dir, analysis_dir, fd_thresh=0.9, overwrite = False, return_censordata_dict = False):
   ...
   TR censor criteria:
    ...
    for key in censordata_dict:
        ...
        file_name = Path(os.path.join(sub_analysis_dir, 'sub-' + sub + "_ses-1_task-sst_" + key + '_censor_' + str(censor_str) + '.1D'))
    ...
    for key in censordata_dict:
        ...
        if key.startswith("run"):
            ...
            summary_dict[key] = {
                'n_trs' : len(censordata_dict[key]),
                'n_uncensored_trs' : sum(censordata_dict[key]),
                'p_uncensored_trs' : round(sum(censordata_dict[key])/len(censordata_dict[key]) ,3)
            }
    ...
    # define output file name
    file_name = Path(os.path.join(sub_analysis_dir, 'sub-' + sub + '_ses-1_task-sst_censor-summary_' + str(censor_str) + '.tsv'))
