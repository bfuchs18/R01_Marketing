...
# make dataframe of counts by risk and protocol for the past week
risk_by_visit_counts <- as.data.frame(xtabs(data = week_visits_long, subset = child_protocol_date > week_start_date, formula = ~ child_protocol_number + risk_status_maternal))

# format dataframe into table
risk_by_visit_counts$child_protocol_number <- gsub("child_protocol_|_date", "", risk_by_visit_counts$child_protocol_number)

table <- risk_by_visit_counts %>% kbl(booktabs = T, col.names = c("Child visit protocol", "Risk status", "Count"))  %>% kable_classic() 
```

Dates: `r week_start_date` to `r date`

**Total visits: `r sum(risk_by_visit_counts$Freq)`**

Table 1. Visit count by protocol and risk status `r table`
